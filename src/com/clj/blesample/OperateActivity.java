...
/**
 * Created by 陈利健 on 2016/9/20. 可作为工具测试
 */
public class OperateActivity extends Activity implements View.OnClickListener {
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.btn_connect:
                ...
                // }
                connectNameDevice("59:C0:61:FE:9B:12");
                break;
    ...
    private void showDeviceList(final BluetoothDevice[] devices) {
        ...
        for (int i = 0; devices != null && i < devices.length; i++) {
            View itemView = LayoutInflater.from(this).inflate(R.layout.layout_list_item_device,
                    null);

            RelativeLayout layout_item_device = (RelativeLayout) itemView
                    .findViewById(R.id.layout_list_item_device);
            TextView txt_item_name = (TextView) itemView.findViewById(R.id.txt_item_name);

            final BluetoothDevice device = devices[i];
            String address = device.getAddress();
            // if (!address.endsWith("12") || !address.endsWith("65")) {
            // continue;
            // }
            txt_item_name.setText(device.getAddress());
    ...
    private void showConnectState(String deviceName, BluetoothGatt gatt) {
        ...
        if (gatt != null) {
            for (final BluetoothGattService service : gatt.getServices()) {
                View serviceView = LayoutInflater.from(this)
                        .inflate(R.layout.layout_list_item_service, null);
                ...
                for (final BluetoothGattCharacteristic characteristic : service
                        .getCharacteristics()) {
                    View characterView = LayoutInflater.from(this)
                            .inflate(R.layout.layout_list_item_character, null);
                    characterView.setTag(characteristic.getUuid().toString());
                    TextView txt_character = (TextView) characterView
                            .findViewById(R.id.txt_character);
                    final Button btn_properties = (Button) characterView
                            .findViewById(R.id.btn_properties);
                            ...
                            btn_properties.setOnClickListener(new View.OnClickListener() {
                                ...
                                public void onClick(View view) {
                                    if (btn_properties.getText().toString().equals("read")) {
                                        startRead(service.getUuid().toString(),
                            ...
                            btn_properties.setOnClickListener(new View.OnClickListener() {
                                ...
                                public void onClick(View view) {
                                    if (btn_properties.getText().toString().equals("notify")) {
                                        startNotify(service.getUuid().toString(),
                                                characteristic.getUuid().toString());
                                    } else if (btn_properties.getText().toString()
                                            .equals("stopListen")) {
                                        stopListen(characteristic.getUuid().toString());
                                        bleManager.stopNotify(service.getUuid().toString(),
                                                characteristic.getUuid().toString());
                                        btn_properties.setText(String.valueOf("notify"));
                                    }
                                }
                            ...
                            btn_properties.setOnClickListener(new View.OnClickListener() {
                                ...
                                public void onClick(View view) {
                                    if (btn_properties.getText().toString().equals("indicate")) {
                                        startIndicate(service.getUuid().toString(),
                                                characteristic.getUuid().toString());
                                    } else if (btn_properties.getText().toString()
                                            .equals("stopListen")) {
                                        stopListen(characteristic.getUuid().toString());
                                        bleManager.stopIndicate(service.getUuid().toString(),
                                                characteristic.getUuid().toString());
                                        btn_properties.setText(String.valueOf("indicate"));
                                    }
                                }
                ...
                new BleCharacterCallback() {
                    ...
                    public void onSuccess(final BluetoothGattCharacteristic characteristic) {
                        ...
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                View characterView = layout_character_list
                                        .findViewWithTag(characterUUID);
                                if (characterView != null) {
                                    TextView txt_value = (TextView) characterView
                                            .findViewById(R.id.txt_value);
                                    if (txt_value != null) {
                                        txt_value.setText(String.valueOf(
                                                HexUtil.encodeHex(characteristic.getValue())));
                                    }
                                }
                            }
                        });
